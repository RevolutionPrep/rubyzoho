...
module ZohoApi
  ...
  class Crm
    ...
    def related_id?(module_name, field_name)
      ...
      return false if module_name.include?("CustomModule") && !field_name.downcase.ends_with?('id')
      return false if field.ends_with?('id')
    ...
    def mass_insert_records(module_name, changes)
      x = REXML::Document.new
      ...
      begin
        ...
        if code = (r["response"]["error"]["code"] rescue false)
          raise(RuntimeError, "Zoho Error Code #{code}: #{r["response"]["error"]["message"]}")
        else
